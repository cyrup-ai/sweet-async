...
use sweet_async_api::task::spawn::{TaskResult, AsyncResult};
...
impl<T: Send + 'static + std::fmt::Debug> AsyncResult<T> for TokioAsyncResult<T> {
        ...
        Box::pin(async move {
            match self.result {
                Ok(_) => {
                    ...
                    TokioTaskResult::new(Err(AsyncTaskError::Failure("and_then not yet implemented".to_string())))
                }
        ...
        Fut: Future<Output = Self> + Send + 'static,
    {
        Box::pin(async move {
            match self.result {
                ...
                Err(_) => {
                    // TODO: implement async work execution
                    TokioAsyncResult::new(Err(AsyncTaskError::Failure("or_else not yet implemented".to_string())))
                }
            }
        })
    }
    ...
    {
        // TODO: implement async work execution
        TokioAsyncResult::new(Err(AsyncTaskError::Failure("map not yet implemented".to_string())))
    }

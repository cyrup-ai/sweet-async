...
use sweet_async_api::task::builder::{
    AsyncTaskBuilder, AsyncWork, ReceiverBuilder as ApiReceiverBuilder, ReceiverStrategy,
    SenderBuilder as ApiSenderBuilder, SenderStrategy,
};
...
use super::event::{TokioEventReceiver, TokioEventSender, create_event_channel};
...
{
    type SenderBuilder<F: AsyncWork<tokio::sync::mpsc::Receiver<T>> + Send + 'static> =
        TokioSenderBuilder<T, C, EItem, EOverall, I, F>;

    fn sender<F_impl>(
        self,
        sender_logic: F_impl,
        strategy: SenderStrategy,
    ) -> Self::SenderBuilder<F_impl>
    where
        F_impl: AsyncWork<tokio::sync::mpsc::Receiver<T>> + Send + 'static,
    {
...
impl<T: Clone + Send + 'static, Task: AsyncTask<T, I>, I: TaskId, E>
    ...
    for TokioEmittingTaskBuilder<T, T, E, I>
where
    E: Send + 'static,
{
...
/// SW is the concrete type implementing AsyncWork<mpsc::Receiver<T>>
pub struct TokioSenderBuilder<T, C, EItem, EOverall, I, SW>
where
    T: Send + 'static,
    C: Send + 'static,
    EItem: Send + Sync + 'static,
    ...
    I: TaskId,
    SW: AsyncWork<tokio::sync::mpsc::Receiver<T>> + Send + 'static,
{
    /// Base builder with common configuration
    base_builder: TokioAsyncTaskBuilder<T, I>,
    /// Tokio runtime handle
...
impl<T, C, EItem, EOverall, I, SW> TokioSenderBuilder<T, C, EItem, EOverall, I, SW>
where
    T: Send + 'static,
    C: Send + 'static,
    EItem: Send + Sync + 'static,
...
impl<
    ...
    EOverall: Send + 'static,
    I: TaskId,
> ApiSenderBuilder<T, C, EItem, I> for TokioSenderBuilder<T, C, EItem, EOverall, I, SW>
where
    T: Send + 'static,
    C: Send + 'static,
    EItem: Send + Sync + 'static,
    ...
    SW: AsyncWork<tokio::sync::mpsc::Receiver<T>> + Send + 'static,
{
    type Receiver = TokioReceiverBuilder<T, C, EItem, I>;
    type StreamProcessorReceiver<Ch, Coll>
        = TokioReceiverBuilder<T, C, EItem, I>
    where
        Ch: Send,
        Coll: Send;
    type EmittingTask = TokioEmittingTask<T, C, EItem, EOverall, I>;
        ...
        <S as futures::Stream>::Item: sweet_async_api::task::emit::ReceiverEvent<T, C>,
    {
        ...
        let receiver_work = move |_t: T| {
            Box::pin(async move { unimplemented!("stream processor not yet implemented") })
        };
...
> TokioEmittingTask<T, C, EItem, AsyncTaskError, I>
{
    ...
    pub fn new(
        ...
        let (receiver_task_cancel_tx, receiver_task_cancel_rx) = oneshot::channel();
        ...
        futures::executor::block_on(async {
            *sender_join_handle_for_storage.lock().await = Some(original_sender_jh.clone());
            let spawned_sender_jh = runtime.spawn(async move {
                let _ = original_sender_jh.await;
            });
            active_tasks_for_sender.lock().await.push(spawned_sender_jh);
        });

        let final_result_for_receiver_task = final_result_arc.clone();
        let runtime_for_receiver = runtime.clone();
        let active_tasks_for_receiver = active_tasks.clone();
        ...
        let original_receiver_jh: JoinHandle<Result<HashMap<Uuid, Result<C, EItem>>, AsyncTaskError>> = runtime_for_receiver.spawn(async move {
            ...
            let _collector_cancel_guard = {
                let token_to_trigger = collector_token.clone();
                async move {
                    receiver_task_cancel_rx.await.ok();
                    token_to_trigger.cancel();
                }
            };
            ...
            tokio::select! {
                ...
                _ = receiver_task_cancel_rx => {
                    ...
                    *final_result_for_receiver_task.lock().await = Some(Ok(collected_map_on_cancel));
        ...
        futures::executor::block_on(async {
            *receiver_join_handle_for_storage.lock().await = Some(original_receiver_jh.clone());
            let spawned_receiver_jh = runtime.spawn(async move {
                let _ = original_receiver_jh.await;
            });
            active_tasks_for_receiver
                .lock()
                .await
                .push(spawned_receiver_jh);
        });
...
impl<T: Clone + Send + 'static, C: Send + 'static, E: Send + 'static, I: TaskId> AsyncTask<T, I>
    for TokioEmittingTask<T, C, E, I>
{
    ...
    EmittingTask<T, C, E, I> for TokioEmittingTask<T, C, E, I>
{
    ...
    fn is_complete(&self) -> bool {
        futures::executor::block_on(async { self.final_result.lock().await.unwrap().is_some() })
    }
    ...
    sweet_async_api::task::CancellableTask<T> for TokioEmittingTask<T, C, E, I>
{
    async fn cancel(
        &self,
        level: sweet_async_api::task::CancellationLevel,
    ...
    fn is_cancelled(&self) -> bool {
        futures::executor::block_on(async { self.final_result.lock().await.unwrap().is_none() })
    }

    ...
    I: TaskId,
{
    ...
    results: Arc<Mutex<HashMap<Uuid, Result<C, EItemProc>>>>,
    ...
    I: TaskId,
{
                    ...
                    maybe_event_data = stream.next() => {
                        if let Some(event_data) = maybe_event_data {
                            let processing_fut = async {
                                ...
                                results_arc.lock().unwrap().insert(event_uuid, processed_result);
                            };
                        ...
                        match stream.next().await {
                            Some(event_data) => {
                                ...
                                let jh = tokio::spawn(async move {
                                    ...
                                    results_inner_arc.lock().unwrap().insert(event_uuid, processed_result);
                                    drop(permit);
                                });
                    ...
                    if !batch_with_ids.is_empty() {
                        ...
                        for (event_uuid, event_data) in std::mem::take(&mut batch_with_ids) {
                            ...
                            results_arc
                                .lock()
                                .unwrap()
                                .insert(event_uuid, processed_result);
                        }
                    }
                    break;
                }
                            ...
                            Some(event_data) => {
                                ...
                                if batch_with_ids.len() >= batch_size {
                                    ...
                                    for (event_uuid_in_batch, event_data_in_batch) in current_batch_to_process {
                                        ...
                                        results_arc.lock().unwrap().insert(event_uuid_in_batch, processed_result);
                                    }
                                    interval.reset();
                                }
                            }
    ...
    fn process_adaptive<S>(
        ...
        T: Send + Sync + 'static,
        C: Send + 'static,
        EItemProc: Send + Sync + 'static + Clone, // Clone for adapted_map_fn if error needs to be part of (Uuid, Result<C,E>)
    {
        let results_arc = self.results.clone();
    ...
    pub fn get_results_map(&self) -> HashMap<Uuid, Result<C, EItemProc>> {

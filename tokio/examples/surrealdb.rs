...
async fn demonstrate_csv_processing() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let csv_result = AsyncTask::emits::<String>()
        .sender(|collector| {
            ...
            collector.of(csv_path.to_string_lossy().to_string())
...
async fn demonstrate_surrealdb_kv(config: &ExampleConfig) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let db_result = AsyncTask::emits::<String>()
        .sender(|collector| {
            ...
            collector.of(config.db_path.to_string_lossy().to_string())
...
mod tests {
    ...
    async fn test_database_setup_teardown() {
        ...
        // Test teardown
        teardown_database(&config).await.expect("Teardown should succeed");
    }
    
    #[tokio::test]
    async fn test_csv_processing() {
        // This test verifies the CSV processing works without errors
        demonstrate_csv_processing().await.expect("CSV processing should succeed");
    }

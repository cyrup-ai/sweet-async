...
macro_rules! await_final_event {
    ...
    ($task:expr, | $event:ident, $collector:ident | {
        ...
        $task.await_final_event(move |$event, $collector| match $event.result() {
            Ok($ok_pat) => $ok_expr,
    ...
    ($task:expr, | $event:ident, $collector:ident | { }) => {{ compile_error!("await_final_event requires OK and ERR patterns") }};
}

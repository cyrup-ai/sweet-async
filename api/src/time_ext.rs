...
impl TimeExt for u64 {
    ...
    fn hours(self) -> Duration { Duration::from_secs(self * 60 * 60) }
    fn days(self) -> Duration { Duration::from_secs(self * 60 * 60 * 24) }
}

impl TimeExt for usize {
    fn nanos(self) -> Duration { (self as u64).nanos() }
    fn micros(self) -> Duration { (self as u64).micros() }
    fn millis(self) -> Duration { (self as u64).millis() }
    fn seconds(self) -> Duration { (self as u64).seconds() }
    fn minutes(self) -> Duration { (self as u64).minutes() }
    fn hours(self) -> Duration { (self as u64).hours() }
    fn days(self) -> Duration { (self as u64).days() }
}

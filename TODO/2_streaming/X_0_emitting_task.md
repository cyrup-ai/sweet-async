# Task: EmittingTask<T, C, E, I> Core Streaming
...
## Description
Implement the core event streaming abstraction for AsyncTask::emits<T>() patterns.

## Success Criteria
- [ ] Fix critical violations in tokio/src/task/emit/task.rs
- [ ] Remove std::process::abort() calls (line 382 in event.rs)
- [ ] Fix broken timestamp methods returning MIN_UTC
- [ ] Implement proper event streaming pipeline
- [ ] Add backpressure handling for stream management

## Dependencies
- 0_foundation/0_asynctask_core.md (core task system)
- 0_foundation/2_asyncresult_core.md (result handling)

## Files to Fix
- tokio/src/task/emit/task.rs
- tokio/src/task/emit/event.rs

## Complexity
High - Core streaming with critical violations

## Production Impact
Critical - Used in all event streaming patterns
